...
import type { Board, CheckItem, Checklist, Column, Comment, Member, Task } from 'src/types/board';
...
const reducers = {
          getBoard(state: BoardState, action: GetBoardAction): void {
          ...
          deleteColumn(state: BoardState, action: DeleteColumnAction): void {
                    const columnId = action.payload;
          ...
          deleteTask(state: BoardState, action: DeleteTaskAction): void {
          ...
          addComment(state: BoardState, action: AddCommentAction): void {
                    const { taskId, comment } = action.payload;
                    const task = state.tasks.byId[taskId];

                    task.comments.push(comment);
          },
          addChecklist(state: BoardState, action: AddChecklistAction): void {
                    const { taskId, checklist } = action.payload;
                    const task = state.tasks.byId[taskId];

                    task.checklists.push(checklist);
          },
          updateChecklist(state: BoardState, action: UpdateChecklistAction): void {
                    const { taskId, checklist } = action.payload;
                    const task = state.tasks.byId[taskId];
          ...
          deleteChecklist(state: BoardState, action: DeleteChecklistAction): void {
