...
/**
 * Fix existing chat rooms to set is_active = true
 * This is a one-time fix for existing rooms that might have is_active as null or false
 */
export const fixExistingChatRooms = async (): Promise<ActionResult<number>> => {
     const supabase = await useServerSideSupabaseAnonClient();
     ...
     try {
          ...
          // Update all rooms to have is_active = true
          ...
          if (updateError) {
               log(`Error updating rooms: ${updateError.message}`);
               return { success: false, error: updateError.message };
          }

          console.log(`Updated ${updatedRooms?.length || 0} rooms to be active`);
          return { success: true, data: updatedRooms?.length || 0 };
     } catch (e: any) {
          log(`Error in fixExistingChatRooms: ${e.message}`);
          return { success: false, error: e.message || 'Unexpected error' };
     }
};
