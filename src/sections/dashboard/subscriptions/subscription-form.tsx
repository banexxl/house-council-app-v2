...
//      // Sync prices when subscriptionPlansData changes
//      useEffect(() => {
//           if (subscriptionPlansData?.prices) {
//                setPrices(subscriptionPlansData.prices);
...
//                // Validate price types - Polar doesn't allow mixing one_time and recurring
...
//                     toast.error("Cannot mix one-time and recurring prices. A product must have either one-time OR recurring prices, not both.");
//                     formik.setSubmitting(false);
//                     return;
//                }

//                if (hasOneTime && prices.filter(p => p.type === 'one_time').length > 1) {
//                     toast.error("Only one one-time price is allowed per product.");
//                     formik.setSubmitting(false);
//                     return;
//                }
...
//           // Determine the type based on existing prices to avoid conflicts
//           const existingType = prices.length > 0 ? prices[0].type : 'recurring';
//           const defaultInterval = existingType === 'recurring' ? 'month' : null;

//           // Check if we already have a one_time price
//           if (existingType === 'one_time' && prices.length > 0) {
//                toast.error("Only one one-time price is allowed. Remove the existing one-time price before adding another.");
//                return;
//           }
...
//                type: existingType,
//                recurringInterval: defaultInterval,
//                recurringIntervalCount: 1,
//                priceCurrency: 'usd',
//                priceAmount: 0,
//                legacy: false,
...
//           // If changing type, validate compatibility with other prices
//           if (field === 'type') {
//                const newType = value;
//                const otherPrices = prices.filter((_, i) => i !== index);

//                if (otherPrices.length > 0) {
//                     const hasConflict = otherPrices.some(p => p.type !== newType);
//                     if (hasConflict) {
//                          toast.error(`Cannot change to ${newType} type. All prices must be the same type. Remove other prices first.`);
//                          return;
//                     }
//                }

//                // If changing to one_time, check if there are already multiple prices
//                if (newType === 'one_time' && prices.length > 1) {
//                     toast.error("Cannot have multiple one-time prices. Remove other prices first.");
//                     return;
//                }

//                // Clear recurringInterval if changing to one_time
//                if (newType === 'one_time') {
//                     updated[index] = { ...updated[index], [field]: value, recurringInterval: null };
//                } else {
//                     updated[index] = { ...updated[index], [field]: value, recurringInterval: 'month' };
//                }
//           } else {
...
//                                              Note: Products can have EITHER recurring prices OR one one-time price, but not both types together.
//                                         </Alert>
//                                    )}
...
//                                                             label={t("subscriptionPlans.recurringIntervalCount")}
//                                                             type="number"
//                                                             value={price.recurringIntervalCount || 1}
//                                                             onChange={(e) => handlePriceChange(index, 'recurringIntervalCount', parseInt(e.target.value) || 1)}
//                                                             slotProps={{ htmlInput: { min: 1, step: "1" } }}
//                                                             margin="dense"
//                                                             helperText="Number of intervals (e.g., 2 for 'every 2 months')"
//                                                        />
//                                                   </>
//                                              )}
//                                         </Card>
